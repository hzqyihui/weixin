...
require_once FCPATH."application/third_party/ymrt/nusoap.php";
require_once FCPATH."application/third_party/ymrt/Mo.php";

class Jibian_message {
	private $CI;
	/**
	 * 网关地址
	 */
	 ...
	 * 往外发送的内容的编码,默认为 GBK
	 */
	private $incomingEncoding = 'UTF-8';

	public function __construct() {
		$this->CI = & get_instance();
		$this->url = $this->CI->config->item('ym_url');
		$this->serialNumber = $this->CI->config->item('ym_serialnumber');
		$this->password = $this->CI->config->item('ym_password');
		$this->sessionKey = $this->CI->config->item('ym_sessionkey');
		$this->namespace = $this->CI->config->item('ym_namespace');
		$this->outgoingEncoding = $this->CI->config->item('ym_outgoingencoding');
		$this->incomingEncoding = $this->CI->config->item('ym_incomingencoding');
			
			
		/**
		 * 初始化 webservice 客户端
		 */	
		$this->soap = new nusoap_client($this->url,FALSE,FALSE,FALSE,FALSE,FALSE,2,10); 
